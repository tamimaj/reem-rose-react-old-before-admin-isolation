...
import { createService } from "../../../../api/private/services";
...
const AddServicesForm = () => {
  ...
  const handleCreatePost = async (values: object) => {
    setLoading(true);
    let response = await createService(values);
    if (!response || response?.status !== 200) {
      setLoading(false);
      toast(<CustomToast message="Service Not Created Successfully" />);
      return;
    }
    setLoading(false);
    toast(
      <CustomToast message="Service Created Successfully" type="success" />
    );
