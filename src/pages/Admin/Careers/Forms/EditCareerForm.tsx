...
const EditCareerForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => handleUpdateCareer(values),
  });

  const handleUpdateCareer = async (values: object) => {
    setLoading(true);
    let response = await updateCareer(values);
    if (!response || response?.status !== 200) {
      setLoading(false);
      toast(<CustomToast message="Career Not Updated Successfully" />);
      return;
    }
    setLoading(false);
    toast(<CustomToast message="Career Updated Successfully" type="success" />);
  ...
  const getCareerData = async () => {
    if (id) {
      ...
      if (response?.data?.careerData) {
        delete response?.data?.careerData?.__v;
        delete response?.data?.careerData?.createdAt;
        delete response?.data?.careerData?.updatedAt;
      }
      ...
      <div className="w-full 3xl:w-[90%] max-w-[1440px] flex flex-col overflow-x-hidden items-center">
        {loading ? (
          ...
          <div className="flex flex-col items-center w-full overflow-y-hidden pb-24">
            <h6 className="text-white font-medium text-xl">Edit Career Form</h6>
