...
const SortMenu: React.FC<sortType> = ({ className }) => {
  ...
  return (
    <div className={`${className} relative`} ref={sortRef}>
      ...
      {sortMenu && (
        <div className="text-base z-50 absolute bg-black text-bodyText w-full top-[46px] rounded  px-1 py-4">
          {" "}
          <button
            onClick={handleClose}
            className="w-full h-[20px] text-[16px] pb-[6px] mb-3  flex items-center justify-between hover:font-semibold hover:text-primary"
          >
          ...
          <button
            onClick={handleClose}
            className="w-full h-[20px] text-[16px] pb-[6px]   flex items-center justify-between hover:font-semibold hover:text-primary"
          >
